        ...
        <div class="col">
            <div class="card">
                <script>
                    ...
                    @else
                        CzechitasTermData[{{ $student->term_id }}] = @json($termService->setContext($student->term)->getNewApplicationTermData());
                        CzechitasTermSelectedId = {{ $student->term_id }};
                    @endif
                </script>
                <form action="{{ routeBack('admin.students.update', $student) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <table class="table table-twocols">
                                        ...
                                        @foreach ($categories as $categoryCollection)
                                            <optgroup label="{{ $categoryCollection->get(0)->category->name }}">
                                                @foreach ($categoryCollection as $term)
                                                    <option value="{{ $term->id }}" {{ oldSelected('term_id', $term->id, $student->term_id) }} data-term-id="{{ $term->id }}">
                                                        {{ $term->term_range }}
                                                    </option>
                                                @endforeach
                                            </optgroup>
                        ...
                        <tr>
                            ...
                            <td>
                                <input id="forename" type="text" class="form-control{{ $errors->has('forename') ? ' is-invalid' : '' }}" name="forename" value="{{ old('forename', $student->forename) }}" required>
                                @error('forename')
                                    <span class="invalid-feedback" role="alert">
                        ...
                        <tr>
                            ...
                            <td>
                                <input id="surname" type="text" class="form-control{{ $errors->has('surname') ? ' is-invalid' : '' }}" name="surname" value="{{ old('surname', $student->surname) }}" required>
                                @error('surname')
                                    <span class="invalid-feedback" role="alert">
                        ...
                        <tr>
                            ...
                            <td>
                                <textarea maxlength="150" name="private_note" class="form-control{{ $errors->has('private_note') ? ' is-invalid' : '' }}" id="private_note" rows="3">{{ old('private_note', $student->private_note) }}</textarea>
                                @error('private_note')
                                    <span class="invalid-feedback" role="alert">
